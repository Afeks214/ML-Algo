...
from ml_algo.pipeline import run_phase3, run_phase4, run_phase5
...
def write_sample_csv(tmp_path: Path, nrows: int = 300) -> Path:
    ...
    path = tmp_path / 'phase5_sample.csv'
    df.to_csv(path, index=False)
    return path


def test_run_phase5_generates_features(tmp_path: Path) -> None:
    ...
    assert result.features.shape[0] == phase3.ha.shape[0]
    assert not result.features.isnull().any().any()
    assert {'score_rq', 'score_periodic', 'score_locally_periodic'}.issubset(result.features.columns)
