...
from ml_algo.pipeline import Phase7Result, run_phase3, run_phase4, run_phase5, run_phase7
...
def write_sample_csv(tmp_path: Path, nrows: int = 300) -> Path:
    ...
    path = tmp_path / "phase7_sample.csv"
    df.to_csv(path, index=False)
    return path


def test_run_phase7_generates_validation_summary(tmp_path: Path) -> None:
    ...
    result = run_phase7(
        ...
        k=3,
        h=1,
        catboost_config=CatBoostConfig(iterations=30, depth=3, learning_rate=0.1),
    )
